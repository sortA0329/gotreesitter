...
func TestApplyReduceActionFromGSSFastPathPreservesVisibleChildOrder(t *testing.T) {
	lang := &Language{
		SymbolCount:    3,
		TokenCount:     2,
		StateCount:     8,
		SymbolNames:    []string{"left", "right", "pair"},
	...
	var gssScratch gssScratch
	gss := newGSSStack(0, &gssScratch)
	gss.push(5, left, &gssScratch)
	gss.push(6, right, &gssScratch)
	s := glrStack{gss: gss}

	act := ParseAction{Type: ParseActionReduce, Symbol: 2, ChildCount: 2, ProductionID: 23}
	tok := Token{Symbol: 0, StartByte: 2, EndByte: 2, StartPoint: Point{Column: 2}, EndPoint: Point{Column: 2}}
	anyReduced := false
	nodeCount := 0
	var entryScratch glrEntryScratch
	var tmpEntries []stackEntry
	...
	if got := len(tmpEntries); got != 0 {
		t.Fatalf("tmpEntries len = %d, want 0", got)
	}

	top := s.top().node
	if top == nil {
		t.Fatal("expected parent node on stack")
	}
	if got, want := top.Symbol(), Symbol(2); got != want {
	...
	if got, want := len(top.children), 2; got != want {
		t.Fatalf("child len = %d, want %d", got, want)
	}
	if top.children[0] != left || top.children[1] != right {
		t.Fatalf("child order mismatch: got [%p %p], want [%p %p]", top.children[0], top.children[1], left, right)
	}
}
